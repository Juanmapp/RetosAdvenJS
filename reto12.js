...
function selectSleigh(distance, sleighs) {
  ...
  let element = sleighs.map((x) => {
    return x.consumption * distance
  })
  let bestSleigh = ""
  console.log(element)
  for ( let i = 0 ; i < element.length ; i++) {
    if (element[i] <= 20 && element[i] < element[i+1]) {
      bestSleigh = sleighs[i]
    }
    else if (element[i] <= 20 && element[i] > element[i-1]) {
      bestSleigh = sleighs[i]
    }
  
  }
  if (element.every((x) => x > 20)) {
    return null
  }
