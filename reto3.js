...
function distributeGifts(packOfGifts, reindeers) {
    ...
    for (let i = 0; i < packOfGifts.length ; i++) {
        giftsWeight = giftsWeight + packOfGifts[i].length
    }
    for (let i = 0; i < reindeers.length ; i++) {
        weightReindeers = weightReindeers + reindeers[i].length
    }
    weightReindeers = weightReindeers * 2
    let rest = weightReindeers % giftsWeight
    let total = (weightReindeers - rest) / giftsWeight
